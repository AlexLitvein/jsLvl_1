...
function handleClick(clickedButtonEvent) {
    ...
    const bttnTxt = card.button.innerText;
    
    if (bttnTxt == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (bttnTxt == 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    const txt = 'вместо картинки над этой кнопкой появлялся какой-нибудь текст';    
